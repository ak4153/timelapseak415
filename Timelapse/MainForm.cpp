...
#include "Mouse.h"
#include <list>
#include "AdvancedRusher.h"
#include "AutoSell.h"
#include "CheckLicense.h"
#include "autocscc.h"
#include <time.h>
#include "SpoofMACHWID.h"
#include <msclr\marshal_cppstd.h>
#include "autodie.h"
#include "DisableHacks.h"
...
ref struct GlobalRefs {
	...
	static int previousLvl;
	static int startingLvl;
	static int totalMesosAcquiredDuringBotting = 0;
	static  int startingMesos ;
	static bool justStartedBotting_MESOS = true;
	static bool justStartedBotting_LVL = true;
	static int previousLvlForAutoAp;
	static int isDC = 1;
	static bool autoPotSwitch = false;
	static int autoSellMap = 0;
	static bool cbcccsFlag = false;
	static bool vacTimerTick = false;
	static int targetExp;
	static int expRushBackMap;
	static bool autoDieSequence = false;
};
...
void MainForm::MainForm_Load(Object^  sender, EventArgs^  e) {
	...
	//Log::WriteLine(writeSpoofedPacket());
	//for (int i = 0; i < 3; i++) {
	//	GlobalRefs::loginPacket = writeSpoofedPacket();
	//}
	tvSendPackets->Nodes->Add("sent packets");
	Log::WriteLineToConsole(":::::::::::::::::::::::::::::::::::::::::");
	Log::WriteLineToConsole(":::         Timelapse Trainer         :::");
	Log::WriteLineToConsole(":::::::::::::::::::::::::::::::::::::::::");
	Log::WriteLineToConsole("Initializing Timelapse trainer Please Stand By ....");
	RECT msRect;
...
void MainForm::MainForm_Shown(Object^  sender, EventArgs^  e) {
	...
	if (File::Exists(Settings::GetSettingsPath())) {
		if (MessageBox::Show("A save file has been detected, load save?", "Save File", MessageBoxButtons::YesNo) == System::Windows::Forms::DialogResult::Yes) {
			...
			//loading item filter into memory
			for (int i = 0; i < lbItemFilter->Items->Count; i++) {
				String^ currItem = lbItemFilter->Items[i]->ToString();
				String^ extractItemId = currItem->Remove(currItem->IndexOf(")"))->Substring(currItem->IndexOf("(") + 1);
				Assembly::itemList->push_back(Convert::ToUInt32(extractItemId));
			}
		}
...
void MainForm::cbPicType_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (this->cbPicType->Checked) {
		WriteMemory(picTypeAddr, 1, 0x90);
	}
	else
		WriteMemory(picTypeAddr, 1, 0x75);
}


//todo rethink the logic, cuz text box shows twice
void MainForm::cb_reconnectDC_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
...
//display instructions for num of clicks for autosell
void MainForm::label120_MouseHover(System::Object^ sender, System::EventArgs^ e) {
	ToolTip^ autoClickSell = gcnew ToolTip();
	autoClickSell->IsBalloon = true;
	autoClickSell->ShowAlways = true;
	autoClickSell->SetToolTip(label120, "Adjust number of clicks if you undersell your inventory");
}
...
void MainForm::GUITimer_Tick(Object^  sender, EventArgs^  e) {
	...
	if (HelperFuncs::IsInGame()) {
		...
		lbEXP->Text = Convert::ToString(PointerFuncs::getCharEXP());
		lbMesos->Text = PointerFuncs::getCharMesos().ToString("N0");

		//autodie check
		if (cb_autoDie->Checked == true && GlobalRefs::autoDieSequence == false) {
			GlobalRefs::autoDieSequence == true;
			if (checkIfNeedToDie(GlobalRefs::targetExp, PointerFuncs::getCharEXP())) {
				disableTimers();
				disableVacs();
				disableMacros();
				autoDie();
				rushBackToMapId(GlobalRefs::expRushBackMap);
				if (GlobalRefs::expRushBackMap == ReadPointer(UIMiniMapBase, OFS_MapID)) {
					GlobalRefs::autoDieSequence == false;
					enableTimers();
					enableVacs();
					enableMacros();
				}
		...
		//sets starting mesos
		if (GlobalRefs::justStartedBotting_MESOS ) {
			GlobalRefs::justStartedBotting_MESOS = false;
			GlobalRefs::startingMesos = PointerFuncs::getCharMesos();
		}
		//sets starting lvl
		if (GlobalRefs::justStartedBotting_LVL) {
			GlobalRefs::startingLvl = readCharValueZtlSecureFuse(*(ULONG*)CharacterStatBase + OFS_Level);
			GlobalRefs::justStartedBotting_LVL = false;

		}
		lb_startingLevel->Text = GlobalRefs::startingLvl.ToString();
		GlobalRefs::totalMesosAcquiredDuringBotting =  PointerFuncs::getCharMesos() - GlobalRefs::startingMesos ;
		lb_mesosCounter->Text = GlobalRefs::totalMesosAcquiredDuringBotting.ToString("N0");
...
void SendLoginPacket(String^ username, String^ password) {
	//todo
	while ((*(BYTE*)(*(ULONG*)LoginBase + OFS_LoginStep) != 1)) {
		MainForm::TheInstance->lb_botStatus->Text = "Auto Logging in";
		String^ packet = "";
...
void SendCharListRequestPacket(int world, int channel) {
	while ((*(BYTE*)(*(ULONG*)LoginBase + OFS_LoginStep) != 2)) {
		////moving to world
		MouseInput::Mouse mouse;
		mouse.moveTo(358, 291, false, false);
		...
		Sleep(1000);
		//move to channel and double click
		mouse.moveTo(372, 416, false, false);
		Sleep(2000);
		mouse.leftClick();
		Sleep(1000);
		//search for correct channel
		int downKeyCount = 0;
		for (int keyPress = 0; channel > keyPress; keyPress++) {
			if (channel == 0) {
				Sleep(500);
				break;
			}
...
void SendSelectCharPacket(int character, bool existsPIC) {
	...
	MouseInput::Mouse mouse;
	if (existsPIC) {
		while ((*(BYTE*)(*(ULONG*)LoginBase + OFS_LoginScreen) != 0)) {

			//Sleep(3000);
			SendPacket(writeSpoofedPacket());

			//String^ PIC = MainForm::TheInstance->tbAutoLoginPIC->Text;
			////moving to character
			//mouse.moveTo(368, 454, false, false);
			//Sleep(5000);
			////double clicking char
			//mouse.doubleLeftClick();
			//Sleep(3000);
			//MainForm::TheInstance->cbPicType->Checked = true;
			////typing pic
			//SendKeys::Send(PIC);
			//MainForm::TheInstance->cbPicType->Checked = false;
			//Sleep(2000);
			//SendKey(VK_RETURN);
			Sleep(5000);
		}
		...
		////SendPacket(packet);
		//Log::WriteLineToConsole(packet);
	}
}

void AutoLogin() {
	if (*(BYTE*)(*(ULONG*)LoginBase + OFS_LoginScreen) == 255 && MainForm::TheInstance->cbAutoLogin->Checked==true) {
...
//		Sleep(Convert::ToUInt32(MainForm::TheInstance->tbCSDelay->Text)*1000);
...
void MainForm::AutoCCCSTimer_Tick(Object^  sender, EventArgs^  e) {
	...
	//		Log::WriteLineToConsole("intruder sent you to CS");
	//		GlobalRefs::cbcccsFlag = true;
	//		if (rbCC->Checked) AutoCC(-1);
	//		else AutoCS();
	//	}
	//}

	if (cb_actionPeople->Checked) {
		if (ReadPointer(UserPoolBase, OFS_PeopleCount) > Convert::ToUInt32(tb_actionPeople->Text)) {
			Log::WriteLineToConsole("intruder sent you to CS");
			autoCsCcPeople(combob_actionPeople->SelectedIndex, Convert::ToUInt16(tb_actionPeopleDelay->Text));
		}
	}


	if ( cb_actionLvlUp->Checked) {
		if (GlobalRefs::previousLvl < readCharValueZtlSecureFuse(*(ULONG*)CharacterStatBase + OFS_Level)) {
			GlobalRefs::previousLvl = readCharValueZtlSecureFuse(*(ULONG*)CharacterStatBase + OFS_Level);
			autoCsCcLVLUP(combob_actionLvlUp->SelectedIndex, Convert::ToUInt16(tb_actionLvlUP->Text));
		}
	}

}
void MainForm::timerActionTime_Tick(System::Object^ sender, System::EventArgs^ e){
	if (cb_actionTime->Checked) {
		autoCsCcTime(combob_actionTime->SelectedIndex, Convert::ToUInt16(tb_actionTime->Text));
	}
...
void MainForm::cb_csAfterLvlUp_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	//left here
	//if (cb_csAfterLvlUp->Checked == true) {
	//	GlobalRefs::previousLvl = readCharValueZtlSecureFuse(*(ULONG*)CharacterStatBase + OFS_Level);
	//}
	//else
	//{
	//	Log::WriteLineToConsole("auto cs after lvl cancelled");
	//}
}
...
Void MainForm::btn_startAutoSell_Click(System::Object^ sender, System::EventArgs^ e){
}
#pragma endregion 
void MainForm::btn_panicButton_Click(System::Object^ sender, System::EventArgs^ e) {
	this->cbAttack->Checked = false;
	this->cbLoot->Checked = false;
	this->cbMP->Checked = false;
	this->cbHP->Checked = false;
	cbKami->Checked = false;
	cbKamiLoot->Checked = false;
	cbSellAll->Checked = false;
	AutoCC(-1);
}

void MainForm::cbSellAll_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (this->cbSellAll->Checked && btn_autoSellInterval->Text != ""&& getMapIdForAutoSell(combobox_townsForAutoSell->SelectedIndex) !=-1) {
		autoSellTimer->Enabled = true;
		autoSellTimer->Stop();
		autoSellTimer->Interval = Convert::ToUInt32(btn_autoSellInterval->Text) * 1000;
		autoSellTimer->Start();
		lb_AutoSellStatus->Text = "Enabled";
		lb_AutoSellStatus->ForeColor = ForeColor.Green;
		GlobalRefs::autoSellMap = getMapIdForAutoSell(combobox_townsForAutoSell->SelectedIndex);
		combobox_townsForAutoSell->Enabled = false;
		btn_autoSellInterval->Enabled = false;
		tb_numOfclicksAutoSell->Enabled = false;
	}
	if (getMapIdForAutoSell(combobox_townsForAutoSell->SelectedIndex) == -1 || btn_autoSellInterval->Text == "" && this->cbSellAll->Checked == true)
	{
		if (getMapIdForAutoSell(combobox_townsForAutoSell->SelectedIndex) == -1) {
			cbSellAll->Checked = false;
			MessageBox::Show("No town selected, select a town");
		}
		if (btn_autoSellInterval->Text == "") {
			cbSellAll->Checked = false;
			MessageBox::Show("Auto Sell interval is invalid");
		}
	}
	if (!this->cbSellAll->Checked) {
		autoSellTimer->Stop();
		autoSellTimer->Enabled = false;
		lb_AutoSellStatus->Text = "Disabled";
		lb_AutoSellStatus->ForeColor = ForeColor.Red;
		combobox_townsForAutoSell->Enabled = true;
		btn_autoSellInterval->Enabled = true;
		tb_numOfclicksAutoSell->Enabled = true;
	}
}

//auto sell sequence 
void MainForm::autoSellTimer_Tick(Object^ sender, EventArgs^ e) {
	if (this->cbSellAll->Checked){
		autoSellTimer->Stop();

		

		//checks if not disconnected or at cashshop
		if (ReadPointer(UIMiniMapBase, OFS_MapID)==0)
		{
			Log::WriteLineToConsole("Char at CashShop, Exiting...");
		...
		Log::WriteLineToConsole("Entering Auto Sell mode");
		MainForm::TheInstance->lb_botStatus->Text = "Entering AutoSell mode";
		//flags for switching back hacks
		bool kamiFlag = false;
		bool kamiLootFlag = false;
		bool dupexFlag = false;

		bool timerActionTimeFlag = false;
		bool autocccspeopleFlag = false;
		bool autocccslvlupFlag = false;

		if (timerActionTime->Enabled)
		{
			timerActionTimeFlag = true;
			timerActionTime->Stop();
		}

		if (this->cb_actionPeople->Checked)
		{
			cb_actionPeople->Checked = false;
			autocccspeopleFlag = true;
		}

		if (this->cb_actionLvlUp->Checked) {
			cb_actionLvlUp->Checked = false;
			 autocccslvlupFlag = true;
		}
		...
		Log::WriteLineToConsole(startMapID.ToString());
		int rushBack = startMapID;
		rushToSell(GlobalRefs::autoSellMap);
		



		Sleep(10000);
		rushBackToMapId(startMapID);

		autoSellTimer->Start();

		Log::WriteLineToConsole("Rushing back to botting map ID:" + " " + startMapID.ToString());
		MainForm::TheInstance->lb_botStatus->Text = "Auto Sell Complete";

		if (autocccslvlupFlag) {
			this->cb_actionLvlUp->Checked = true;
		}

		if (autocccspeopleFlag) {
			this->cb_actionPeople->Checked = true;
		}

		if (kamiFlag == true) {
			cbKami->Checked = true;
			this->cbAttack->Checked = true;
			this->cbLoot->Checked = true;
			this->cbMP->Checked = true;
			this->cbHP->Checked = true;
		}
		if (kamiLootFlag == true) {
			cbKamiLoot->Checked = true;
			this->cbAttack->Checked = true;
			this->cbLoot->Checked = true;
			this->cbMP->Checked = true;
			this->cbHP->Checked = true;
		...
		if (timerActionTimeFlag) {
			this->timerActionTime->Start();
			timerActionTimeFlag = false;
		}
...
void MainForm::itemVacTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
	if (!GlobalRefs::vacTimerTick) {
		Jump(itemVacAddr, Assembly::ItemVacHook, 2);
		GlobalRefs::vacTimerTick = true;
		itemVacTimer->Interval = (Convert::ToInt32(tb_vacTimerInterval->Text) * 1000) * 0.1;
	}
	else if (GlobalRefs::vacTimerTick) {
		WriteMemory(itemVacAddr, 7, 0x50, 0xFF, 0x75, 0xDC, 0x8D, 0x45, 0xCC);
		GlobalRefs::vacTimerTick = false;
		itemVacTimer->Interval = (Convert::ToInt32(tb_vacTimerInterval->Text) * 1000) ;
	}
}
...
void MainForm::cbItemVac_CheckedChanged(Object^  sender, EventArgs^  e) {
	if (this->cbItemVac->Checked) {
		/*Jump(itemVacAddr, Assembly::ItemVacHook, 2);*/
		itemVacTimer->Interval = Convert::ToInt32(tb_vacTimerInterval->Text) * 1000;
		tb_vacTimerInterval->Enabled = false;
		itemVacTimer->Enabled = true;
		itemVacTimer->Start();
	}
		
	else {
		/*WriteMemory(itemVacAddr, 7, 0x50, 0xFF, 0x75, 0xDC, 0x8D, 0x45, 0xCC);*/
		itemVacTimer->Enabled = false;
		tb_vacTimerInterval->Enabled = true;
	}
...
static void findItemsStartingWithStr(String^ str) {
	try {
		...
		while (File.good()) {
			...
			String^ a = gcnew String(tmpStr.c_str());
			String^ b = gcnew String(itemStr.c_str());
...
void MainForm::cb_itemFilter_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
...
void MainForm::bItemFilterAdd_Click(System::Object^  sender, System::EventArgs^  e) {
	if(tbItemFilterID->TextLength > 0) {
		try {
			...
			bool existInListFlag = false;
			for (int i = 0; i < lbItemFilter->Items->Count; i++) {
				if (lbItemFilter->Items->Contains(item + " (" + itemID.ToString() + ")") == lbItemFilter->Items[i]->ToString()){
					existInListFlag = true;
					Log::WriteLineToConsole(lbItemFilter->Items[i]->ToString());
					}
			}
			if(itemID > 0 && existInListFlag==false) {
				lbItemFilter->Items->Add(item + " (" + itemID.ToString() + ")");
				tbItemFilterID->Text = "";
				lbItemFilter->SelectedIndex = -1;
				Assembly::itemList->push_back(itemID);
				existInListFlag = false;
			}
		}
...
void MainForm::lbItemSearchLog_MouseDoubleClick(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) {
	if(lbItemSearchLog->SelectedItem != nullptr && lbItemSearchLog->SelectedItem->ToString()->Length > 0&& !lbItemFilter->Items->Contains(lbItemSearchLog->SelectedItem)) {
...
void MainForm::tbItemFilterSearch_TextChanged(System::Object^  sender, System::EventArgs^  e) {
	lbItemSearchLog->Items->Clear();
	//NewThread(findItemsStartingWithStr, tbItemFilterSearch->Text);
...
void MainForm::bSendLog_Click(System::Object^  sender, System::EventArgs^  e) {
	if(bSendLog->Text->Equals("Enable Log")) {
		...
		/*Jump(CinPacketAddr, Assembly::RecievePacketLogHook, 0);*/
...
void MainForm::tPacketLog_Tick(System::Object^  sender, System::EventArgs^  e) {
	...
	if(GlobalRefs::bSendPacketLog && !Assembly::sendPacketQueue->empty()) {
		COutPacket *packet = Assembly::sendPacketQueue->front();
		//CInPacket *packet1 = Assembly::recvPacketQueue->front();

		String^ packetData = "";
		String^ charID;
		String^ spoofedlogin;
		//BYTE* Bytes = Assembly::packetLogSend->Data;
		//int packetSize = packet->Size - 2;
		//for (int i = 0; i < packetSize; i++)
		//	writeByte(packetData, Bytes[i]);
		////Log::WriteLineToConsole(packetData);
		if (ConvertStdToSystemStr(Assembly::packetlogSendStr)->Contains("1E 00")) {
			charID = ConvertStdToSystemStr(Assembly::packetlogSendStr);
			charID = charID->Substring(30, 5);
			charID = charID->Replace(" ", "");
			Log::WriteLineToConsole(charID);
			Timelapse::MainForm::TheInstance->tb_charID->Text = charID;
			Timelapse::MainForm::TheInstance->tbSendPacket->Text = writeSpoofedPacket();
		}

		Timelapse::MainForm::TheInstance->tvSendPackets->Nodes[0]->Nodes->Add(ConvertStdToSystemStr(Assembly::packetlogSendStr));
		
		Assembly::sendPacketQueue->pop();
		//Assembly::recvPacketQueue->pop();
...
void existsInNextMapDFS(int currMapID, int startMapID, int destMapID, int numRecursions, cliext::vector<MapPath^>^ searchList, cliext::vector<MapPath^>^ finalPath,bool &destFound,std::list<int> &visited) {

//	Log::WriteLine(getMap(currMapID)->mapName);
//	Log::WriteLine(numRecursions.ToString());
	...
	if (currMapID == destMapID) {
		...
		//Log::WriteLine("return 2 hit");
		return; //Returning so that no further maps from this one are searched
	}

	if (getMap(currMapID)->portals->Count == 0 || numRecursions > 100 || destFound) {
		//Log::WriteLine("return 1 hit");
		return;
	}
	...
	for each(PortalData^ portalData in getMap(currMapID)->portals) {
		...
		for each (double mapID in visited) {
			if (currMapID == mapID) {
				return;
			}
		...
		existsInNextMapDFS(portalData->toMapID, startMapID, destMapID, numRecursions + 1, searchList, finalPath,destFound,visited); //Recursive call
		searchList->pop_back();
	}
	visited.push_back(currMapID);
}
...
cliext::vector<MapPath^>^ generatePath(int startMapID, int destMapID) {
	...
	existsInNextMapDFS(startMapID, startMapID, destMapID, 0, searchList, finalPath, destFound, visited); //Gets shortest path and puts it into finalPath (if there exists a path)
	return finalPath;
}
...
int rushNextIsland(int startMapID, int destMapID) {
	...
	switch (startIsland) {
		...
		case 10://if startIsland is victoria
			if (destIsland == 11) {
				return rushToVictoriaFromFlorina(startMapID);
			}
			if (destIsland == 20) {			
				return rushToOrbisFromEllinia(startMapID);
			}
			if (destIsland == 22) {
				int middleIsland = rushToOrbisFromEllinia(startMapID);
				return rushToLudiFromOrbis(middleIsland);
			}

		case 11:
		return	rushToFlorinaFromVictoria(startMapID);
		...
		case 20://if startIsland is orbis
			if (destIsland == 10) {
				return rushToElliniaFromOrbis(startMapID);
			}
			if (destIsland == 22) {
				return rushToLudiFromOrbis(startMapID);
			}

		case 22://if startIsland is ludi
			if (destIsland == 20) {
				return rushToOrbisFromLudi(startMapID);
			}
			if (destIsland == 10) {
				int middleIsland = rushToOrbisFromLudi(startIsland);
				return rushToElliniaFromOrbis(middleIsland);
			}
	}
	return 0;
}
...
bool existsInterIslandPath(int startMapID, int destMapID) {
	...
	switch(startIsland) { 
		case 0:
			...
			break;
		case 10:
			if (destIsland == 11|| destIsland == 20|| destIsland == 22) return true;
			break;
		case 11:
			if (destIsland == 10) return true;
			break;
		case 20:
			if (destIsland == 10 || destIsland == 22 || destIsland == 21)return true;
			break;
		case 22:
			if (destIsland == 20 || destIsland == 10)return true;
	}
...
static void mapRush(int destMapID) {
	...
	if (getIsland(startMapID) != getIsland(destMapID) && getIsland(destMapID)!=21) {
	...
	for (auto i = mapPath->begin(); i != mapPath->end(); ++i, j++) {
		...
		if (i == mapPath->begin()) {
			...
			if (oldChannel == 1) AutoCC(-1); else AutoCC(-1);
			Sleep(delay);
		}
...
void  MainForm::cb_Enable_Repot_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {

	if (cb_Enable_Repot->Checked == true) {
		autoPotTimer->Enabled = true;
		autoPotTimer->Start();
		GlobalRefs::autoPotSwitch = true;
		lb_autoRePotStatus->Text = "Enabled";
		lb_autoRePotStatus->ForeColor = ForeColor.Green;;
	}

	else {
		autoPotTimer->Stop();
		autoPotTimer->Enabled = false;
		GlobalRefs::autoPotSwitch = false;
		lb_autoRePotStatus->Text = "Disabled";
		lb_autoRePotStatus->ForeColor = ForeColor.Red;
	}
}

//autorepot 
void MainForm::autoPotTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
	if (GlobalRefs::autoPotSwitch == true &&tb_User_Defined_HP->Text != "" && tb_User_Defined_MP->Text != "" && AutoRepotTown(comboBox_AutoRepotTown->SelectedIndex)!=0) {
		int mapid = comboBox_AutoRepotTown->SelectedIndex;

		int currMapId = Convert::ToInt32(PointerFuncs::getMapID());
		//checks if pots in inv are lower than what expected and rushes to buy
		if ( ReadPointer(AutoPotBase, OFS_PageUpKey) < Convert::ToInt32(tb_User_Defined_MP->Text)
		|| ReadPointer(AutoPotBase, OFS_HomeKey) < Convert::ToInt32(tb_User_Defined_HP->Text)) {
			autoPotTimer->Stop();
			//gets town to sell from users input
			...
			mapRush(AutoRepotTown(mapid));
			int hpPotToBuy=0, mpPotToBuy=0;
			//checks pots tobuy
			if (ReadPointer(AutoPotBase, OFS_HomeKey) < Convert::ToInt32(tb_User_Defined_HP->Text)) {
				 hpPotToBuy = Convert::ToInt32(tb_HP_tobuy->Text);
			}
			if (ReadPointer(AutoPotBase, OFS_PageUpKey) < Convert::ToInt32(tb_User_Defined_MP->Text)) {

				 mpPotToBuy = Convert::ToInt32(tb_MP_tobuy->Text);
			}
				BuyPotions(ReadPointer(UIMiniMapBase, OFS_MapID), hpPotToBuy, mpPotToBuy);
			mapRush(currMapId);
...
void MainForm::checkBox1_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (cb_autoAP->Checked) {
		autoAPTimer->Enabled = true;
		GlobalRefs::previousLvlForAutoAp = Convert::ToUInt32(PointerFuncs::getCharLevel());
	}
	else
	{
		autoAPTimer->Enabled = false;
	}
}
void MainForm::autoAPTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
	//todo: add something so it wont add sp in cashsop
	if (Convert::ToUInt32(PointerFuncs::getCharLevel()) < Convert::ToUInt32(tb_AutoAPUILevel->Text)&&
		GlobalRefs::previousLvlForAutoAp< Convert::ToUInt32(PointerFuncs::getCharLevel())&& PointerFuncs::getMapID!=000000000) {
		int str = Convert::ToUInt32(tb_UIStr->Text);
		int dex = Convert::ToUInt32(tb_UIDex->Text);
		int intel = Convert::ToUInt32(tb_UIInt->Text);
		int luk = Convert::ToUInt32(tb_UILuk->Text);
		AutoAPAllocation(str,dex,intel,luk);
		GlobalRefs::previousLvlForAutoAp = Convert::ToUInt32(PointerFuncs::getCharLevel());
		Log::WriteLineToConsole("Allocated STR:{0}, DEX:{1}, INT{2}, LUK:{3}");
	}
}

void MainForm::dcTimer_Tick(System::Object^ sender, System::EventArgs^ e) {
	try
	{
		if (*(BYTE*)(*(ULONG*)LoginBase + OFS_LoginScreen) == 255 && cbAutoLogin->Checked) {
			Sleep(5000);
			SendKey(VK_RETURN);
			AutoLogin();
			Log::WriteLineToConsole("Succefully relogged.");
		}
	}
	catch (...)
	{
...
void MainForm::cb_actionLvlUp_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (tb_actionLvlUP->Text != "" && combob_actionLvlUp->SelectedItem->ToString() != "") {
		GlobalRefs::previousLvl = readCharValueZtlSecureFuse(*(ULONG*)CharacterStatBase + OFS_Level);
		//autoCsCcLVLUP(combob_actionLvlUp->SelectedIndex, Convert::ToUInt16(tb_actionLvlUP->Text));
	}
	else
...
void MainForm::cb_actionPeople_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
		if (tb_actionPeople->Text == "" || combob_actionPeople->SelectedItem->ToString() == "" || tb_actionPeopleDelay->Text == "") {
			cb_actionPeople->Checked = false;
		}
}
void MainForm::cb_actionTime_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (cb_actionTime->Checked == true) {
		timerActionTime->Interval = (Convert::ToUInt16(tb_actionTimeTimerInterval->Text))*1000;
		timerActionTime->Enabled = true;
		timerActionTime->Start();
	}
	if (cb_actionTime->Checked == false) {
		timerActionTime->Enabled = true;
		timerActionTime->Start();
	}
}
void MainForm::cb_autoDie_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	if (cb_autoDie->Checked == true && tb_targetExp->Text!="") {
		tb_targetExp->Enabled = false;
		GlobalRefs::targetExp = Convert::ToUInt16(tb_targetExp->Text);
		GlobalRefs::expRushBackMap = ReadPointer(UIMiniMapBase, OFS_MapID);
		GlobalRefs::autoDieSequence == false;
	}
	else
		tb_targetExp->Enabled = true;
}
#pragma endregion
